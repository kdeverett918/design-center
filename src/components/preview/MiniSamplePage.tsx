...
export default function MiniSamplePage({ brand, headline }: MiniSamplePageProps) {
  const copy = useCopy();
  const head = headline ?? copy.headline;
  return (
    <div className="flex h-full flex-col bg-bg text-ink font-body">
      ...
      <div className="flex items-center justify-between border-b tk-line px-4 py-2.5">
        ...
        <div className="flex items-center gap-2 text-[8px] text-muted">
          <span>{copy.navLinks[0]}</span>
          <span>{copy.navLinks[1]}</span>
          <span className="rounded-full bg-primary px-2 py-0.5 text-onPrimary">{copy.navCta}</span>
        </div>
      </div>
      ...
      <div className="relative flex-1 px-4 pb-3 pt-4">
        ...
        <div className="mt-2.5 flex items-center gap-1.5">
          <span className="inline-flex items-center gap-1 rounded-lg bg-primary px-2.5 py-1 text-[8px] font-semibold text-onPrimary">
            {copy.ctaPrimary} <ArrowRight size={8} />
          </span>
